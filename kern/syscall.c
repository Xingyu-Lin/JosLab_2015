...
sys_exofork(void)
{
    ...
    struct Env* e;
    int errno;
    if ((errno = env_alloc(&e, curenv->env_id)) != 0) return errno;
...
sys_page_alloc(envid_t envid, void *va, int perm)
{
	...
	// LAB 4: Your code here.
    if (((unsigned int)va >= UTOP )|| (((unsigned int)va & 0xfff) !=0)) return -E_INVAL;
	     ...
	     envid_t dstenvid, void *dstva, int perm)
{
    ...
    pte_t *addr;
    struct PageInfo *curpage;
    int errno;
    if ((unsigned int) srcva>= UTOP || ((unsigned int)srcva & 0xfff)) return -E_INVAL;
    if ((unsigned int) dstva>= UTOP || ((unsigned int)dstva & 0xfff)) return -E_INVAL;
    if (((perm & PTE_U)==0) || ((perm & PTE_P) ==0) || ((perm & (~PTE_SYSCALL)) !=0)) return -E_INVAL;
    if ((errno=envid2env(srcenvid, &srcenv, 1))<0) return errno;
    if ((errno=envid2env(dstenvid, &dstenv, 1))<0) return errno;

    curpage = page_lookup(srcenv->env_pgdir, srcva, &addr);
    if ((curpage == NULL) || (((perm & PTE_W) !=0) && ((*addr & PTE_W) ==0)) )
            return -E_INVAL;
    if ((errno = page_insert(dstenv->env_pgdir, curpage, dstva, perm)) !=0) return errno;
    return 0;
}
...
sys_page_unmap(envid_t envid, void *va)
{
	...
	// LAB 4: Your code here.
    struct Env* e;
    if ((unsigned int)va >= UTOP || ((unsigned int) va & 0xfff)) return -E_INVAL;
    if (envid2env(envid, &e, 1)!=0) return -E_BAD_ENV;
...
sys_ipc_try_send(envid_t envid, uint32_t value, void *srcva, unsigned perm)
{
    ...
    if ((int) srcva < UTOP)
    {
            int mask = PTE_U | PTE_P |PTE_W | PTE_AVAIL;
            if ((perm & PTE_U) ==0 || (perm & PTE_P) ==0 || (perm & ~mask) >0)
                    return -E_INVAL;
    }

    pte_t* pte;
    struct PageInfo* pp = page_lookup(curenv->env_pgdir, srcva, &pte);
    if ((int) srcva < UTOP && pp==NULL) return -E_INVAL;
    if ((int) srcva < UTOP && (*pte & PTE_W) ==0 && (perm & PTE_W) !=0) return -E_INVAL;
    if ((int) srcva < UTOP && e->env_ipc_dstva!=0)
    {
...
{
	// LAB 4: Your code here.
    if ((int)dstva < UTOP && ROUNDDOWN(dstva, PGSIZE)!=dstva) return -E_INVAL;
...
//syscall to load a new program as unix-exec
static int
...
syscall(uint32_t syscallno, uint32_t a1, uint32_t a2, uint32_t a3, uint32_t a4, uint32_t a5)
{
    ...
    case SYS_env_set_status:
        ret = (int) sys_env_set_status((envid_t) a1, (int) a2);
        break;
    case SYS_page_map:
